...
import { updateBoxSchema } from "@/app/lib/zod";
...
export default function EditBoxForm({ box, _id }: { box: Box; _id: string }) {
  ...
  const handleSubmit = async (values: z.infer<typeof updateBoxSchema>) => {
    const response = await editBox(
      { ...values, pdf_link: values.pdf_link ? values.pdf_link : [] },
      _id
    );

    if (response.status === 200) {
      router.push(`/dashboard/cajas/${values.symbol}`);
      Toast.fire({
        icon: "success",
        title: "Se ha editado la caja correctamente",
      });
    } else {
      Toast.fire({
        icon: "error",
        title: "Error al editar la información de la caja",
      });
    }
  };
